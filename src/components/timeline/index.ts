Component({
  ...
  data: {
    expandedItems: {} as Record<string, boolean>, // 记录每个item的展开状态
    taskCheckedStatus: {} as Record<string, boolean>, // 记录每个task的选中状态，key为"itemId-taskIndex"
    computedItems: [] as any[], // 计算后的items,包含动态状态
    noteExpanded: {} as Record<string, boolean>, // 记录每个备注区域的展开状态
    itemNotes: {} as Record<string, string> // 记录每个item的备注内容
  },
  ...
  lifetimes: {
    attached() {
      ...
      if (this.data.computedItems && this.data.computedItems.length > 0) {
        ...
        (this.data.computedItems as any[]).forEach((item: any) => {
  ...
  methods: {
    ...
    initNoteExpandedStatus() {
      const noteExpanded: Record<string, boolean> = {};
      const items = this.data.items as any[];
    ...
    // 检查某个item是否有选中的task
    hasCheckedTask(itemId: string) {
      const items = this.data.items as any[];
      const item = items.find((i: any) => i.id === itemId);
      if (!item || !item.tasks) return false;
